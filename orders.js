                ...
                if (historySheets.length > 0) {
                    const historyResults = await Promise.all(
                        historySheets.map(sheetName =>
                            fetch(`https://sheets.googleapis.com/v4/spreadsheets/${process.env.SHEET_ID}/values/${encodeURIComponent(sheetName)}?key=${process.env.GOOGLE_API_KEY}`)
                                .then(r => r.json())
                                .catch(() => ({ values: [] }))
                        )
                    );
